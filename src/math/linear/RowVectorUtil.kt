...
// */
fun <T: Ring<T>> RowVector<T>.elementMatrixAt(columnIndex: Int): T = get(columnIndex)

///**
// * Returns an element at the given [index] or the result of calling the [defaultValue] function if the [index] is out of bounds of this list.
// */
/*inline*/ fun <T: Ring<T>> RowVector<T>.elementMatrixAtOrElse(columnIndex: Int, defaultValue: (Int) -> T): T =
    getOrElse(columnIndex, defaultValue)

///**
// * Returns an element at the given [index] or `null` if the [index] is out of bounds of this list.
// */
fun <T: Ring<T>> RowVector<T>.elementMatrixAtOrNull(columnIndex: Int): T? = getOrNull(columnIndex)

/**
 * Returns a matrix containing the results of applying the given [transform] function
 * to each element in the original matrix.
 */
fun <T: Ring<T>, S: Ring<S>> RowVector<T>.mapMatrix(transform: (T) -> S): RowVector<S> =
 ...
 * to each element in the original matrix.
 */
fun <T: Ring<T>, S: Ring<S>> RowVector<T>.mapMatrixIndexed(transform: (columnIndex: Int, T) -> S): RowVector<S> =
 ...
 * to each element in the original matrix.
 */
fun <T: Ring<T>, S: Ring<S>> RowVector<T>.mapMatrixIndexed(transform: (rowIndex: Int, columnIndex: Int, T) -> S): RowVector<S> =
...
// */
/*inline*/ fun <T: Ring<T>, R> RowVector<T>.foldMatrixIndexed(initial: R, operation: (columnIndex: Int, acc: R, T) -> R): R {
...
// * and performs the action on the element.
// */
/*inline*/ fun <T: Ring<T>> RowVector<T>.onMatrixEachIndexed(action: (columnIndex: Int, T) -> Unit): Matrix<T> = apply { forMatrixEachIndexed(action) }
...
// * and performs the action on the element.
// */
/*inline*/ fun <T: Ring<T>> RowVector<T>.onMatrixEachIndexed(action: (rowIndex: Int, columnIndex: Int, T) -> Unit): Matrix<T> = apply { forMatrixEachIndexed(action) }
