...
fun <T: Field<T>> Matrix<T>.leftGaussianEliminationWithTransformation(): LeftGaussianEliminationResult<T> {
    ...
    val transformationCoefs = MutableList(countOfRows) { rowIndex -> MutableList(countOfRows) { columnIndex -> if (rowIndex == columnIndex) ONE else ZERO } }
    val inverseTransformationCoefs = MutableList(countOfRows) { rowIndex -> MutableList(countOfRows) { columnIndex -> if (rowIndex == columnIndex) ONE else ZERO } }
    ...
    for (columnNow in 0 until countOfColumns) {
        ...
        if (coolRow != countOfProcessedRows) {
            resultCoefficients[coolRow] = resultCoefficients[countOfProcessedRows].also { resultCoefficients[countOfProcessedRows] = resultCoefficients[countOfProcessedRows] }
            transformationCoefs[coolRow] = transformationCoefs[countOfProcessedRows].also { transformationCoefs[countOfProcessedRows] = transformationCoefs[countOfProcessedRows] }
            inverseTransformationCoefs[coolRow] = inverseTransformationCoefs[countOfProcessedRows].also { inverseTransformationCoefs[countOfProcessedRows] = inverseTransformationCoefs[countOfProcessedRows] }
        }
...
fun <T: Field<T>> Matrix<T>.rightGaussianEliminationWithTransformation(): RightGaussianEliminationResult<T> {
    ...
    val resultCoefficients = MutableList(countOfRows) { rowIndex -> MutableList(countOfColumns) { columnIndex -> coefficients[columnIndex][rowIndex] } }
    val transformationCoefs = MutableList(countOfRows) { rowIndex -> MutableList(countOfRows) { columnIndex -> if (rowIndex == columnIndex) ONE else ZERO } }
    val inverseTransformationCoefs = MutableList(countOfRows) { rowIndex -> MutableList(countOfRows) { columnIndex -> if (rowIndex == columnIndex) ONE else ZERO } }
...
// * Returns an element at the given [index] or the result of calling the [defaultValue] function if the [index] is out of bounds of this list.
// */
inline fun <T: Ring<T>> Matrix<T>.matrixElementAtOrElse(rowIndex: Int, columnIndex: Int, defaultValue: (Int, Int) -> T): T =
    getOrElse(rowIndex, columnIndex, defaultValue)

///**
// * Returns an element at the given [index] or the result of calling the [defaultValue] function if the [index] is out of bounds of this list.
// */
inline fun <T: Ring<T>> Matrix<T>.matrixElementAtOrElse(index: Pair<Int, Int>, defaultValue: (Pair<Int, Int>) -> T): T =
...
// */
@Suppress("NOTHING_TO_INLINE")
inline fun <T: Ring<T>> Matrix<T>.matrixElementAtOrNull(rowIndex: Int, columnIndex: Int): T? = getOrNull(rowIndex, columnIndex)
