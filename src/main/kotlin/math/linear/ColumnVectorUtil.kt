...
// */
fun <T: Ring<T>> ColumnVector<T>.elementMatrixAt(rowIndex: Int): T = get(rowIndex)

///**
// * Returns an element at the given [index] or the result of calling the [defaultValue] function if the [index] is out of bounds of this list.
// */
/*inline*/ fun <T: Ring<T>> ColumnVector<T>.elementMatrixAtOrElse(rowIndex: Int, defaultValue: (Int) -> T): T =
    getOrElse(rowIndex, defaultValue)

///**
// * Returns an element at the given [index] or `null` if the [index] is out of bounds of this list.
// */
fun <T: Ring<T>> ColumnVector<T>.elementMatrixAtOrNull(rowIndex: Int): T? = getOrNull(rowIndex)

/**
 * Returns a matrix containing the results of applying the given [transform] function
 * to each element in the original matrix.
 */
fun <T: Ring<T>, S: Ring<S>> ColumnVector<T>.mapMatrix(transform: (T) -> S): ColumnVector<S> =
 ...
 * to each element in the original matrix.
 */
fun <T: Ring<T>, S: Ring<S>> ColumnVector<T>.mapMatrixIndexed(transform: (rowIndex: Int, T) -> S): ColumnVector<S> =
 ...
 * to each element in the original matrix.
 */
fun <T: Ring<T>, S: Ring<S>> ColumnVector<T>.mapMatrixIndexed(transform: (rowIndex: Int, columnIndex: Int, T) -> S): ColumnVector<S> =
...
// */
/*inline*/ fun <T: Ring<T>, R> ColumnVector<T>.foldMatrixIndexed(initial: R, operation: (rowIndex: Int, acc: R, T) -> R): R {
...
// * and performs the action on the element.
// */
/*inline*/ fun <T: Ring<T>> ColumnVector<T>.onMatrixEachIndexed(action: (rowIndex: Int, T) -> Unit): Matrix<T> = apply { forMatrixEachIndexed(action) }
...
// * and performs the action on the element.
// */
/*inline*/ fun <T: Ring<T>> ColumnVector<T>.onMatrixEachIndexed(action: (rowIndex: Int, columnIndex: Int, T) -> Unit): Matrix<T> = apply { forMatrixEachIndexed(action) }
